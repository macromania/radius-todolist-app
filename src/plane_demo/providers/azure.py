...
from plane_demo.providers.commands import Commands, create_json, write_json, write_private
from plane_demo.providers.credentials import Credentials, database_dsn
from plane_demo.provisioning import Cluster, OperatorConfig, ProvisioningError, endpoint, plain
...
CONTAINER_CERTIFICATE_COMMAND = ("python", "/app/scripts/run-certificate-job.py")
...
class AzureProvider:
    ...
    def register_cluster_environment(self, slot: str) -> str:
        ...
        self.rad(
            "management",
            "deploy",
            str(self.root / "infra/radius/environments/project-azure.bicep"),
            "--group",
            "radplanes",
            "--parameters",
            f"@{parameter_file}",
    ...
    def deploy(
        ...
        *,
        ...
        self.rad(
            slot,
            "deploy",
            str(self.root / "infra/radius/apps" / f"{template}.bicep"),
            "--group",
            "radplanes",
            "--environment",
            environment or slot,
    ...
    def bootstrap_child(self, cluster: Cluster) -> None:
        ...
        self.commands.run(
            [
                sys.executable,
                str(self.root / "scripts/install-radius.py"),
                "--context",
                cluster.context,
                "--kubeconfig",
    ...
    def certificate(self, slot: str, domain: str) -> str:
        ...
        default = (
            ...
            else (sys.executable, str(self.root / "scripts/run-certificate-job.py"))
        )
