        ...
        "clusters": [
            {
                ...
                "cluster": {
                    "server": server,
                    "certificate-authority-data": encoded("synthetic-ca"),
...
class Offline:
    def __init__(self, state, targets):
            ...
            self.resources["management"].append(
                {
                    ...
                    "properties": {
                        ...
                        "application": (
                            f"{cleanup.SCOPE}/Applications.Core/applications/cluster-{slot}"
                        ),
                        "provisioningState": "Succeeded",
        ...
        for name in ("management-api", "provisioner"):
            deployment = {
                "metadata": {
                    "name": name,
                    "namespace": targets["management"]["namespace"],
                    "uid": name + "-uid",
                    "labels": {
                    ...
                    "template": {
                        "spec": {
                            "containers": [
                                {
                                    "name": name,
                                    "image": IMAGES["api" if name == "management-api" else name][
                                        "reference"
                                    ],
                                }
                            ]
    ...
    def child_state(self, slot, index):
        ...
        state = {
            ...
            "serial": target["terraformState"]["serial"],
                    ...
                    "instances": [
                        {
                            "attributes": {
                                "name": name,
                                "id": f"{name}-{cleanup.NODE_IMAGE}",
                                "node_image": cleanup.NODE_IMAGE,
                                "completed": True,
                                "kubeconfig": kubeconfig,
                                "client_key": "synthetic-key-not-real",
                            }
                        }
                    ],
                },
        ...
        secret = {
            "metadata": {
                "name": target["terraformState"]["secret"],
                "uid": target["terraformState"]["uid"],
    ...
    def native_delete(self, instance, slot):
        ...
        self.resources["management"] = [
            value
            for value in self.resources["management"]
        ...
        if args[0] == "rad":
            ...
            if command[:2] == ["app", "delete"]:
                ...
                self.mutations.append(
                    (
                        "radius-app",
                        slot if not name.startswith("cluster-") else name,
                    )
                )
                if not self.app_sticks:
                    ...
                    removed = [
                        value
                        for value in self.resources[slot]
                ...
                return json.dumps(
                    {
                        "items": [
                            obj
                            for (s, ns, kind, _), obj in self.objects.items()
...
def scenario(local_state, monkeypatch):
    ...
    for index, slot in enumerate(cleanup.SLOTS):
        ...
        if slot != "management":
            targets[slot].update(
                terraformState={
                    "secret": CLUSTER_STATE_NAMES[slot],
                    "uid": "state-" + slot,
    ...
    monkeypatch.setattr(
        cleanup,
        "load_inputs",
        lambda: (targets, {"source_revision": REVISION, "images": IMAGES}),
    )
    monkeypatch.setattr(
        cleanup.Cleanup,
        "native_delete",
...
@pytest.mark.parametrize("wire_casing", ["resourceGroups", "resourcegroups"])
def test_full_cleanup_uses_owners_and_independent_read_only_verify(scenario, capsys, wire_casing):
    state, _, fake = scenario
    original = fake.run
...
def test_all_ownership_checked_before_any_mutation(scenario, failure):
    ...
    if failure == "cluster-uid":
        ...
        fake.nodes["e" * 64] = {
            "Id": "e" * 64,
            "Name": "/radplanes-local-foreign-control-plane",
...
def test_no_direct_child_or_unrelated_container_delete_in_operator():
    ...
    assert '"kind", "delete", "cluster", "--name", MANAGEMENT' in " ".join(source.split())
...
def real_inputs(state, targets):
    config = {
        ...
        "recipes": {
            role: {
                "digest": "sha256:" + "c" * 64,
                "moduleServer": "local-module-" + "c" * 20,
                "reference": (
                    "http://local-module-"
                    + "c" * 20
    ...
    common.write_private(
        ...
        (state / "home/.kube/config").read_text(),
    )
    bootstrap = {
        "name": cleanup.MANAGEMENT,
        "context": cleanup.MANAGEMENT,
        "secretEncryptionVerified": True,
        "nodeId": targets["management"]["nodeId"],
        "nodeAddress": "172.18.0.2",
    }
    ...
    for name, value in (
        ("provisioning.json", config),
        ("acceptance.json", export),
        ("management-created.json", bootstrap),
        ("runtime-images.json", images),
    ):
...
@pytest.mark.parametrize(
    "case,live_rule,should_fail",
    [
        ("none", False, False),
        ("not-restored", False, True),
        ("none", True, True),
        ("restored", False, False),
        ("old-error", False, False),
        ("pre-mutation-failure", False, False),
        ("missing-strategy", False, True),
        ("wrong-strategy", False, True),
        ("missing-identity", False, True),
        ("missing-probe", False, True),
        ("changed-pod", False, True),
        ("changed-node", False, True),
...
def test_fault_restoration_is_checked_from_real_runpath(
    ...
    def node(slot):
        return {
            "id": targets[slot]["nodeId"],
            "address": targets[slot]["nodeAddress"],
    ...
    if case != "none":
        record = {
            ...
            "pod_uid": str(UUID(int=201)),
            "cluster_uid": targets["shared-data"]["clusterUid"],
            "namespace_uid": targets["shared-data"]["namespaceUid"],
            "creation_attempted": True,
            "restored": True,
            "physical_restored": True,
            "original_rules_sha256": "c" * 64,
            "restored_rules_sha256": "c" * 64,
            "restoration_probe": {"ok": True},
            "node": node("shared-data"),
            "sandbox": sandbox(),
        }
...
def test_full_recipe_image_owner_must_match_before_any_mutation(scenario, change):
    ...
    secret = fake.objects[
        ...
        "radius-system",
        "secret",
        targets[slot]["terraformState"]["secret"],
    ]
