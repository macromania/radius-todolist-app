...
Status: Validated (reset verified; fresh management deployment running).
...
of the failed Recipe's orphan resources. Fresh management is now deploying.
...
locked Recipes; it was activated only after reset verification. Wait for
`deploy-management-fresh` completion and verify its endpoint/worker before
starting a fresh three-tenant scenario. The bootstrap uses standalone Bicep through the scoped
...
not claim tenant acceptance or whole-environment teardown.
