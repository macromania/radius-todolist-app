...
def management_config(encryption_path: str, vm_socket: str) -> dict:
            ...
            "extraVolumes": [
                {
                    ...
                    "hostPath": encryption_in_node,
                    "mountPath": encryption_in_node,
                    "readOnly": True,
                    "pathType": "File",
                }
            ],
        ...
        "nodes": [
            {
                ...
                "kubeadmConfigPatches": [yaml.safe_dump(patch)],
                "extraMounts": [
                    {
                        "hostPath": vm_socket,
                    ...
                    {
                        "hostPath": encryption_path,
                        "containerPath": encryption_in_node,
                        "readOnly": True,
                    },
                ],
                "extraPortMappings": [
...
def create(commands: Commands, vm_socket: str) -> None:
    ...
    try:
        ...
        write_private(STATE / "management/encryption.yaml", yaml.safe_dump(encryption))
        config = management_config(str(STATE / "management/encryption.yaml"), vm_socket)
...
def install(commands: Commands) -> None:
    ...
    patch = yaml.safe_load((ROOT / "operations/local/dynamic-rp-overlay.yaml").read_text())
