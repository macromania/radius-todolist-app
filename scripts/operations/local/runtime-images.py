...
if component=="api":
    if any((root/path).exists() for path in (
        "src/plane_demo/management/provisioner.py","src/plane_demo/management/providers","scripts"
    )):
...
def expected_hashes(role: str) -> dict[str, str]:
    ...
    if role == "api":
        ...
        paths += [
            ...
            *(ROOT / "scripts/operations" / name for name in OPERATOR_FILES),
...
def rootfs_proof(path: Path, role: str, arch: str) -> dict:
    ...
    with tarfile.open(path, "r:") as archive:
        for member in archive:
            ...
            if role == "api" and (
                name.startswith(("app/scripts/", "app/src/plane_demo/management/providers/"))
