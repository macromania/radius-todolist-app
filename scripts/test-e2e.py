...
_spec = importlib.util.spec_from_file_location(
    "plane_demo_fault_helper", ROOT / "scripts/fault-parent-link.py"
)
...
def source_files(component):
    if component != "provisioner":
        return sorted(
            ...
            + [
                "src/plane_demo/__init__.py",
                "src/plane_demo/bootstrap.py",
                "sql/management.sql",
                "sql/control.sql",
            ]
        )
    paths = [
        ...
        *ROOT.glob("infra/radius/types/*.yaml"),
        ...
        ROOT / "infra/radius/environments/project-azure.bicep",
        *(ROOT / "scripts" / name for name in COORDINATOR_SCRIPTS),
    ]
            ...
            require(
                not faults.command(
                    [
                        ...
                        "infra/radius/apps",
                        "scripts/test-e2e.py",
                        "scripts/fault-parent-link.py",
                        "scripts/export-state.py",
                        "containers/api",
                        "containers/provisioner",
                        "pyproject.toml",
                        "uv.lock",
